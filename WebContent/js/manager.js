  ...
  ;(function($){  
    $.fn.extend({
		...
		setContent = function(){	
			...
			for ( var a in jsonObject.manager) {//获取有几个person
				var tr = document.createElement("tr");
				tr.setAttribute('id','tr1');
				...
				$("tr#tr1:last").append(" <input type='checkbox' name='zhuangtai'>");
				$("</td>").appendTo(tr);
                for ( var item in jsonObject.manager[a]) {  
                    if(item=="managerAccount") 
                    	var p1 = jsonObject.manager[a][item];//得到属性值的内容 
                	if(item=="schoolId") 
                		var p2 = jsonObject.manager[a][item];//得到属性值的内容
                	if(item=="managerName") 
                		var p3 = jsonObject.manager[a][item];//得到属性值的内容
                }
