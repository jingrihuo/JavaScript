...
<body>
	...
	function timedCount(id) {
			...
			xmlhttp.onreadystatechange = function() {
				if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
					...
					$(document).Table('.table_body', 're', count, 5, sd,
							[ '序号', '选中情况', '班级编号', '班级名称', '班级描述' ],
							[ [ , , ], [ , , ], [ , , ] ]);
					$(document).PaginBar('page_navigation', 'table_body', 20);
				}
			}
			var url = "webservlet?method=searchclass&manageraccount=<%=request.getParameter("manageraccount")%>";
		...
		/* function sendAjax() {
			...
			formData.append('classid', $("#f1").value);
			formData.append('classname', $("#f2").value);
			...
			//创建一个 post 请求，采用异步
			xhr.open('POST', 'webservlet?method=searchclass', true);
			//注册相关事件回调处理函数
		...
		function sendAjax() {
			...
			var url = "webservlet?method=searchclass&manageraccount=<%=request.getParameter("manageraccount")%>"+"&classid="+document.getElementById("f1").value+"&classname="+document.getElementById("f2").value;
 ...
 */
			...
			var json ="{" +  "\"manageraccount\"" +":"
					+"\"<%=request.getParameter("manageraccount")%>\"" +","+ "\"classid\"" + ":"
					+ "["
			...
			$.ajax({
				...
				dataType : null,
				url : "webservlet?method=deleteclass",//传入后台的地址/方法
		...
		function fn3(){
			window.open("classform_updateclass.jsp?manageraccount=<%=request.getParameter("manageraccount")%>&classid="+$(":checkbox:checked").next('td').text()+"&classname="+$(":checkbox:checked").next('td').next('td').text()+"&classdetail="+$(":checkbox:checked").next('td').next('td').next('td').text(),"_blank"); 
		}
		function fnn(){
...
<div id="content" class="col-lg-10 col-sm-10">
	<div class="box-inner" >
	 ...
	 <!-- 查询 -->
				...
				班级编号：<input type="text" name="classid" id="f1" onblur="fnn()"> 班级名称：<input
					type="text" name="classname" id="f2" onblur="fnn2()"> <input type="hidden"
					name="manageraccount" id="hidden1" value="">
				<script>
					$(".hidden1").val = $("span.hidden-sm hidden-xs").text();
				</script>
				<input type="submit" value="查询" onclick="sendAjax()">
				<a href="daoru.jsp?manageraccount=<%=request.getParameter("manageraccount")%>">导入excel表</a>
			 <!-- 增加 --><br> <a
			href="classform_addclass.jsp?manageraccount=<%=request.getParameter("manageraccount")%>" target="_Blank"><input
				type="button" value="增加班级"></a> <!-- 修改 --> 
				<input
				type="button" value="修改班级" onclick="fn3()"> 
				<span>
				注意修改班级的时候只能勾选一个复选框！
				</span><br>
				<input type="button"
				value="批量选中" onclick="manycheck()"> 
				<input type="button"
				value="批量取消" onclick="removecheck()">
			 	<input type="button"
				value="删除" onclick="deleteclass()">
