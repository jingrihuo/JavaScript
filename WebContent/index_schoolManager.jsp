...
<body>
	...
	<script type="text/javascript">
			...
			xmlhttp.onreadystatechange = function() {
				if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
					...
					$(document).Table('.table_body', 're', count, 5, sd,
							[ '序号', '选中情况','学校编号', '学校名称', '学校描述' ],
							[ [ , , ], [ , , ], [ , , ] ]);
					$(document).PaginBar('page_navigation', 'table_body', 20);
				}
			}
			var url = "webservlet?method=searchschool&manageraccount=<%=request.getParameter("manageraccount")%>";
		...
		/*function sendAjax() {
			...
			formData.append('schoolid', $("#f1").value);
			formData.append('schoolname', $("#f2").value);
			formData.append('schooldetai', $("#hidden1").value);
			//创建xhr对象 
			...
			//创建一个 post 请求，采用异步
			xhr.open('POST', 'webservlet?method=searchschool', true);
			//注册相关事件回调处理函数
		...
		function sendAjax() {
			...
			var url = "webservlet?method=searchschool&schoolid="+document.getElementById("f1").value+"&schoolname="+document.getElementById("f2").value+"&schooldetai="+document.getElementById("hidden1").value;
 ...
 */
			...
			$.ajax({
				...
				dataType : null,
				url : "webservlet?method=deleteschool",//传入后台的地址/方法
		...
		function fn3(){
			window.open("classform_updateschool.jsp?manageraccount=<%=request.getParameter("manageraccount")%>&schoolid="+$(":checkbox:checked").next('td').text()+"&schoolname="+$(":checkbox:checked").next('td').next('td').text()+"&schooldetail="+$(":checkbox:checked").next('td').next('td').next('td').text(),"_blank"); 
		}
	</script>
</head>
			...
			<div class="nav-canvas">
				<div class="nav-sm nav nav-stacked"></div>
				 <ul class="nav nav-pills nav-stacked main-menu">
                        ...
                        <li><a class="ajax-link" href="index_schoolManager.jsp?manageraccount=<%=request.getParameter("manageraccount")%>&username=<%=request.getParameter("username")%>"><i class="glyphicon glyphicon-eye-open"></i><span>学校管理</span></a>
                        </li>
                          <li><a class="ajax-link" href="index_adminManager.jsp?manageraccount=<%=request.getParameter("manageraccount")%>&username=<%=request.getParameter("username")%>"><i
                                    class="glyphicon glyphicon-align-justify"></i><span> 管理员管理</span></a></li>
                     </ul> 
			</div>
		</div>
	</div>
	...
	<div class="box-inner" >
		<div class="row">
		 ...
		 <!-- 查询 -->
				...
				学校编号：<input type="text" name="schoolid" id="f1"> 
				学校名称：<input
					type="text" name="schoolname" id="f2">
				学校描述： <input type="text"
					name="schooldetail" id="hidden1" value="">
				<input type="submit" value="查询" onclick="sendAjax()">
				<a href="daoru_school.jsp">导入excel表</a>
			 <!-- 增加 --> <a
			href="classform_addschool.jsp?manageraccount=<%=request.getParameter("manageraccount")%>" target="_Blank"><input
				type="button" value="增加学校"></a> <!-- 修改 --> 
				<input
				type="button" value="修改学校信息" onclick="fn3()"> <span>
				注意修改学校信息的时候只能勾选一个复选框！
				</span>
				<br>
				<input type="button"value="批量选中" onclick="manycheck()">
			    <input type="button"value="批量取消" onclick="removecheck()">
			    <input type="button"value="删除" onclick="deleteschool()">
			</div>
		</div>
	<span id="div1">
