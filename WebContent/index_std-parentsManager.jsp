...
<script src="js/std-parents.js"></script>
...
<body>
	...
	<script type="text/javascript">
			...
			xmlhttp.onreadystatechange = function() {
				if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
					...
					$(document).Table('.table_body', 're', count, 10, sd,
							[ '序号', '选中情况', '班级编号', '班级名称',  '学生学号', '学生姓名','学生性别' ,'家长姓名','家长手机号', '家长性别'],
							[ [ , , ], [ , , ], [ , , ] ]);
					$(document).PaginBar('page_navigation', 'table_body', 20);

				}
			};
			var url = "webservlet?method=searchstudent&manageraccount=<%=request.getParameter("manageraccount")%>";
		...
		/*function sendAjax() {
			...
			//创建一个 post 请求，采用异步
			xhr.open('POST', 'webservlet?method=searchstudent', true);
			//注册相关事件回调处理函数
			xhr.onreadystatechange = function() {
				if (this.status == 200 || this.status == 304) {
					...
					var i = 0;
					$(document).Table('.table_body', 're', count, 10, sd,
							[ '序号', '选中情况', '学生学号', '学生姓别',  '学生姓别', '家长编号','家长姓名' ,'家长性别','班级编号', '班级名称'],
							[ [ , , ], [ , , ], [ , , ] ]);
					$(document).PaginBar('page_navigation', 'table_body', 20);
		...
		function sendAjax() {
			...
			var url = "webservlet?method=searchstudent&studentid="+document.getElementById("f1").value+"&studentname="+document.getElementById("f2").value+"&classid="+document.getElementById("f3").value+"&parentname="+document.getElementById("f4").value+"&manageraccount="+document.getElementById("hidden1").value;
		...
		function deletestd() {
            ...
            str3 = $(":checkbox:checked").map(function() {
                ...
                return $(this).next('td').next('td').next('td').next('td').next('td').next('td').next('td').text();  // 根据checkbox定位到后面的td，然后使用text()函数获取内容
 ...
 */
			...
			var json ="{" +  "\"manageraccount\"" +":"
					+"\"<%=request.getParameter("manageraccount")%>\"" +","+ "\"student\"" + ":"
					+ "["
			...
			$.ajax({
				...
				dataType : null,
				url : "webservlet?method=deletestudent",//传入后台的地址/方法
		...
		function fn3(){
			window.open("classform_updatestd.jsp?manageraccount=<%=request.getParameter("manageraccount")%>&studentid="+$(":checkbox:checked").next('td').next('td').next('td').text()+"&classid="+$(":checkbox:checked").next('td').text()+"&studentname="+$(":checkbox:checked").next('td').next('td').next('td').next('td').text()
			+"&useraccount="+$(":checkbox:checked").next('td').next('td').next('td').next('td').next('td').next('td').next('td').text()+"&parentname="+$(":checkbox:checked").next('td').next('td').next('td').next('td').next('td').next('td').text(),"_blank"); 
		}
	</script>
</head>
			...
			<div class="nav-canvas">
				<div class="nav-sm nav nav-stacked"></div>
				 <ul class="nav nav-pills nav-stacked main-menu">
                        ...
                        <li><a class="ajax-link" href="index_classManager.jsp?manageraccount=<%=request.getParameter("manageraccount")%>&username=<%=request.getParameter("username")%>"><i class="glyphicon glyphicon-eye-open"></i><span>班级管理</span></a>
                        </li>
...
<div id="content" class="col-lg-10 col-sm-10">
	<div class="box-inner" >
		...
		<!-- 查询 -->
				...
				学生编号：<input type="text" name="studentid" id="f1">
				学生姓名：<input
					type="text" name="studentname" id="f2"> 
				学生所在班级编号：<input type="text" name="classid" id="f3">
				家长姓名：<input type="text" name="parentname" id="f4">
					<input type="hidden"
				...
				<a href="daoru_std.jsp">导入excel表</a>
			 <!-- 增加 --> <a
			href="classform_addstd.jsp?manageraccount=<%=request.getParameter("manageraccount")%>" target="_Blank"><input
				type="button" value="增加学生-家长"></a> <!-- 修改 --> 
				<input
				type="button" value="修改学生-家长信息" onclick="fn3()">
				<span>注意修改学生-家长信息的时候只能勾选一个复选框！</span>
				<br>
				<input type="button"value="批量选中" onclick="manycheck()"> 
				<input type="button"value="批量取消" onclick="removecheck()">
				<input type="button"value="删除" onclick="deletestd()">	
			</div>
		</div>
	<span id="div1">
